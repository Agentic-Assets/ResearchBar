---
summary: "Provider data sources and parsing overview for every registered ResearchBar provider."
read_when:
...
ResearchBar currently registers 67 provider IDs. Some companies expose multiple surfaces, such as Codex vs OpenAI API or
...
Each native currency has its own total, subscription/model ranking, and daily chart. ResearchBar never adds or ranks
...
- Add Account and Switch Account open Cursor's authenticator in a supported browser; Switch Account prefers stable account IDs and falls back to normalized email when IDs are unavailable. ResearchBar uses the supported system HTTPS handler when possible and otherwise asks the user to choose an eligible supported browser.
...
## Droid (Factory)
- API key from `~/.config/researchbar/config.json` (`providers[].apiKey`), `FACTORY_API_KEY`, or `~/.factory/.env`.
...
- Auto-imports ZoomMate/Zoom session cookies from Chrome, validates and stores the narrowed cookie
  header in ResearchBar's Keychain cache, and exchanges it for a short-lived bearer token through
...
- External status page: `https://status.synthetic.new` (not linked or auto-polled by ResearchBar).
...
## ClinePass
- API key from `~/.config/researchbar/config.json`, `CLINE_API_KEY`, or `CLINEPASS_API_KEY`.
...
- Ordinary CLI/test runs do not import browser cookies unless `CODEXBAR_ALLOW_BROWSER_COOKIE_IMPORT=1` is set;
  `researchbar cookie refresh --provider grok` opts in for its explicit refresh.
...
## GroqCloud
- API key from `~/.config/researchbar/config.json` or `GROQ_API_KEY`; base URL override via `GROQ_API_URL`.
...
## LLM Proxy
- API key + base URL from `~/.config/researchbar/config.json` (`enterpriseHost`), `LLM_PROXY_API_KEY`, or `LLM_PROXY_BASE_URL`.
...
## ClawRouter
- API key from the resolved ResearchBar config (`providers[].apiKey`) or `CLAWROUTER_API_KEY`.
...
- Provider rows are data-driven, so any routed provider returned by ClawRouter is displayed without provider-specific ResearchBar code.
