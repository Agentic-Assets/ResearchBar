...
ResearchBar can use either a DeepSeek API key or a signed-in DeepSeek Platform session for the remaining credit balance.
...
ResearchBar resolves the Platform `userToken` in this order:
...
ResearchBar checks every Chrome profile containing a parseable `userToken` against DeepSeek. Rejected or expired
sessions are omitted. Settings shows a **Chrome profile** picker containing only valid sessions. With no API key, one
valid session is selected automatically; with multiple valid sessions, ResearchBar reuses the saved choice or asks once.
When an API key supplies the balance, a new or changed API credential requires an explicit session selection before
website usage is combined with it. ResearchBar persists a stable browser/profile identifier, not an absolute home-directory path, and
...
If the selected session expires, ResearchBar asks before switching to another valid profile.
...
- The API separates granted balance from topped-up balance; ResearchBar labels these as granted vs. paid credit.
...
- The amount and cost requests run concurrently. After balance arrives, ResearchBar waits up to five seconds for
...
- If total balance is zero, ResearchBar shows an add-credits message. If balance is nonzero but `is_available` is false, it shows "Balance unavailable for API calls".
- There is no session or weekly window — DeepSeek does not expose per-window quota via API.
- Token-account selection injects the selected key into the fetch environment; otherwise ResearchBar reads `DEEPSEEK_API_KEY` / `DEEPSEEK_KEY`.
