...
event's `CODEXBAR_*` variables; ResearchBar provider keys and tokens are not inherited. The same event is also encoded as
...
The v1 field names and meanings are compatibility-stable. Hook consumers should ignore unknown fields so ResearchBar can
...
`researchbar`, and a cookie written by tooling is shown in the app after reload.
...
printf '%s' "$CLAWROUTER_API_KEY" | researchbar config set-api-key --provider clawrouter --stdin
printf '%s' "$SUB2API_API_KEY" | researchbar config set-api-key --provider sub2api --stdin
printf '%s' "$AIAND_API_KEY" | researchbar config set-api-key --provider aiand --stdin
