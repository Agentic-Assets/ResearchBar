...
1) **Cached cookie header** (Keychain cache `com.corbis.researchbar.cache`, account `cookie.factory`).
2) **Stored session** (`~/Library/Application Support/ResearchBar/factory-session.json`).
...
## Session storage
- File: `~/Library/Application Support/ResearchBar/factory-session.json`
