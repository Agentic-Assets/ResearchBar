...
When developing ResearchBar, you may see frequent keychain permission prompts like:

> **ResearchBar wants to access key "Claude Code-credentials" in your keychain.**

This happens because each rebuild creates a new code signature, and macOS treats it as a "different" app.
That can affect both ResearchBar-owned entries (`com.corbis.researchbar`, `com.corbis.researchbar.cache`) and
...
This creates a self-signed certificate named "ResearchBar Development".

#### 2. Trust the Certificate

1. Open **Keychain Access.app**
2. Find **"ResearchBar Development"** in the **login** keychain
3. Double-click it
...
```bash
export APP_IDENTITY='ResearchBar Development'
```
...
> Note: `compile_and_run.sh` now auto-detects a valid signing identity (Developer ID or ResearchBar Development).
...
If you see multiple `ResearchBar *.app` bundles in your project directory, you can clean them up:

```bash
# Remove all numbered builds
rm -rf "ResearchBar "*.app
...
The build script creates `ResearchBar.app` in the project root. Old numbered builds (like `ResearchBar 2.app`) are created when Finder cannot overwrite the running app.
...
This script:
1. Kills existing ResearchBar instances from this repo
...
Keychain access for that process to avoid repeated password prompts. Use the packaged `ResearchBar.app` when local
...
When the script falls back to ad-hoc signing, it preserves ResearchBar-owned keychain state by default.
That means you may still see keychain prompts for existing ResearchBar cache entries, but allowing those prompts keeps the
cached browser/OAuth state available across normal rebuilds.
If you want a clean reset of ResearchBar-owned keychain state for an ad-hoc build, run
...
### "ResearchBar is already running"
