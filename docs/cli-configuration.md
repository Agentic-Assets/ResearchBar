---
summary: "ResearchBar CLI configuration commands for provider toggles, API keys, and isolated config files."
read_when:
  - Using researchbar config from scripts or CI
...
New installs use `~/.config/researchbar/config.json`; absolute `XDG_CONFIG_HOME` paths and `RESEARCHBAR_CONFIG` are
supported. `CODEXBAR_CONFIG` is accepted only as an explicit legacy override, so ResearchBar does not auto-load a
separate CodexBar install's config file.
...
These are persistent app/CLI settings. They are different from `researchbar usage --provider grok`, which is a one-shot
command override and does not edit config.

If every provider is disabled, `researchbar usage` with no `--provider` prints no text output, and
`researchbar usage --json` prints `[]`. Passing `--provider <name>` still fetches that provider for the one command.
...
```bash
printf '%s' "$OPENROUTER_API_KEY" | researchbar config set-api-key --provider openrouter --stdin --no-enable
```
...
printf '%s' "$ANTHROPIC_ADMIN_KEY" | researchbar config set-api-key --provider claude --stdin
printf '%s' "$DEEPGRAM_API_KEY" | researchbar config set-api-key --provider deepgram --stdin
...
printf '%s' "$Z_AI_API_KEY" | researchbar config set-api-key --provider zai --stdin
```
...
sessions instead of an xAI API key for ResearchBar's billing view, so enable them with
`researchbar config enable --provider grok`.

LLM Proxy also needs a base URL. Use `LLM_PROXY_BASE_URL` for CLI runs, or add `"enterpriseHost"` to the provider entry
in the ResearchBar config file.
...
```bash
researchbar cost --provider codex --days 90
researchbar cost --provider claude --days 180 --format json --pretty
```
