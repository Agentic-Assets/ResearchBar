...
read_when:
  - "You want to call ResearchBar data from scripts or a terminal."
...
- `researchbar` defaults to the `usage` command.
  ...
  - JSON uses the generic `usage.details` array for provider-specific information. Each section contains an optional
    ...
    shape is returned by `GET /usage` from `researchbar serve`.
  - Legacy provider-specific keys such as `openRouterUsage`, `clawRouterUsage`, and `sub2APIUsage` are not compatibility
    ...
    when decoding.
- `researchbar cost` prints token cost usage for Claude, Codex, and Cursor.
...
- `researchbar cards` prints a one-shot usage snapshot as a responsive terminal card grid.
  - Reuses the same provider, source, account, credits, and status flags as `researchbar usage`.
  ...
  - This precedence is cards-only: `researchbar usage` and `researchbar serve` keep their existing output cardinality.
...
- `researchbar dashboard` prints one dashboard-v1 JSON snapshot and exits.
...
- `researchbar serve` starts a foreground HTTP server for usage and cost JSON plus a token-gated dashboard snapshot.
  ...
  - `GET /health` returns `{"status":"ok"}` plus a `version` field with the running build (e.g. `"0.37.2"`) when resolvable; clients can compare it against `researchbar --version` to detect a `serve` process still running an older binary after an update.
...
- `researchbar cookie refresh` ignores the provider's current cookie caches while importing a replacement through its web strategy. A failed or interrupted import leaves existing cookies intact.
...
- `researchbar guard --provider <id>` gates automation on one provider's remaining quota.
  ...
  - Guard fetches are read-only and use background interaction policy, matching `researchbar usage`; they never request interactive Keychain access.
...
- `researchbar hooks list` shows the local hook configuration; `--format json` and `--pretty` are supported.
- `researchbar hooks enable|disable` changes the explicit top-level opt-in switch in the local config file.
- `researchbar hooks test <event> --provider <id>` invokes matching enabled rules with a representative event. Hook
...
- `researchbar hooks watch` polls enabled providers and fires matching hooks on real quota and status transitions.
  ...
  - Runs read-only, like `researchbar guard`: it never prompts for credentials. A failed refresh reports
...
researchbar cost                     # cost usage (default 30-day window + today)
researchbar cost --days 90           # choose a 1...365 day cost window
researchbar cost --provider codex --group-by project
researchbar cost --provider claude --format json --pretty
researchbar guard --provider codex --min-remaining 20 --window weekly --json
researchbar cost --provider cursor   # Cursor dashboard cost (API-rate + Cursor-metered)
researchbar dashboard | jq '.providers[] | {id, windows, error}'
...
CODEXBAR_DASHBOARD_TOKEN=YOUR_TOKEN researchbar serve # token-gated dashboard snapshot
CODEXBAR_DASHBOARD_TOKEN=... researchbar serve --host 0.0.0.0 --allow-plain-http # LAN, cleartext accepted
...
For `researchbar dashboard`, `0` includes a valid partial snapshot whose provider rows contain errors. The command exits
...
- OpenAI web requires a signed-in `chatgpt.com` session in a supported browser or a manual cookie header. No passwords are stored; ResearchBar reuses cookies.
- Safari cookie import may require granting ResearchBar Full Disk Access (System Settings → Privacy & Security → Full Disk Access).
