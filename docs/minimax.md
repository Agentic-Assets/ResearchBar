...
1) **Coding Plan API token**
   - Set in Preferences → Providers → MiniMax (stored in `~/.config/researchbar/config.json`), `MINIMAX_CODING_API_KEY`,
     or `MINIMAX_API_KEY`.
...
4) **Manual session cookie header** (optional web-path override)
   - Stored in `~/.config/researchbar/config.json` via Preferences → Providers → MiniMax (Cookie source → Manual).
