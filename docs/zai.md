...
- Check:
  ...
  researchbar usage --provider zai --account Team
  ```
...
For BigModel China team usage, ResearchBar needs the `Bigmodel-Organization` and `Bigmodel-Project` request headers:
...
- Endpoint overrides must be explicit HTTPS URLs or bare hosts/paths that ResearchBar normalizes to HTTPS. Explicit
...
- Live API checks return success with empty limits when one of the selectors is missing, so ResearchBar treats both
...
- ResearchBar's Usage Dashboard action follows the configured API region.
