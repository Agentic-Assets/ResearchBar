---
summary: "ResearchBar release checklist: package, sign, notarize, appcast, and asset validation."
read_when:
  - Starting a ResearchBar release
  - Updating signing/notarization or appcast steps
  - Validating release assets or Sparkle feed
---

# Release process (ResearchBar)

SwiftPM-only; package/sign/notarize manually (no Xcode project). Sparkle feed is served from GitHub Releases after a ResearchBar feed URL is approved.

**Must read first:** open the master macOS release guide at `~/Projects/agent-scripts/docs/RELEASING-MAC.md` alongside this file and reconcile any differences in favor of ResearchBar specifics before starting a release.

## Expectations
- When someone says “release ResearchBar”, do the entire end-to-end flow: bump versions/CHANGELOG, build, sign and notarize, upload the zip to the GitHub release, generate/update the appcast with the new signature, publish the tag/release, and verify the enclosure URL responds with 200/OK and installs via Sparkle after the feed is enabled.
...
```
./Scripts/build_icon.sh Icon.icon ResearchBar
```
...
- [ ] Homebrew tap: only applicable after a ResearchBar cask exists. Do not reuse CodexBar's cask.
...
- [ ] Release pages: title format `ResearchBar <version>`, notes as Markdown list (no stray blank lines)
- [ ] Changelog/release notes are user-facing: avoid internal-only bullets (build numbers, script bumps) and keep entries concise
- [ ] Download uploaded `ResearchBar-macos-universal-<ver>.zip`, unzip via `ditto`, run, and verify signature (`spctl -a -t exec -vv ResearchBar.app` + `stapler validate`)
...
- [ ] Keep a previous signed build in `/Applications/ResearchBar.app` to test Sparkle delta/full update to the new release after the feed is enabled.
- [ ] Manual Gatekeeper sanity: after packaging, `find ResearchBar.app -name '._*'` is empty, `spctl --assess --type execute --verbose ResearchBar.app` and `codesign --verify --deep --strict --verbose ResearchBar.app` succeed
- [ ] For Sparkle verification: if replacing `/Applications/ResearchBar.app`, quit first, replace, relaunch, and test update
- **Definition of “done” for a release:** all of the applicable items above are complete, the appcast/enclosure link resolves, and a previous public build can update to the new one via Sparkle after the feed is enabled. Anything short of that is not a finished release.
