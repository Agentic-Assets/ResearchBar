...
| App executable | `CodexBar` | `ResearchBar` | `Scripts/package_app.sh`; local launch scripts |
...
| Widget extension | `CodexBarWidget.appex` | `ResearchBarWidget.appex`; executable `ResearchBarWidget`; bundle id `com.corbis.researchbar.widget` | `Scripts/package_app.sh`; `WidgetExtension/project.yml` |
...
| CLI command | `codexbar` | `researchbar` | `Scripts/package_app.sh`; `bin/install-researchbar-cli.sh`; Preferences -> Advanced |
...
1. `Scripts/package_app.sh`: bundle id, app group, generated bundle name, packaged executable name, helper names, widget extension name, and output bundle name. **Done for ResearchBar.**
...
5. `Sources/CodexBar/About.swift`, `CodexbarApp.swift`: remaining human-visible strings. **Done for ResearchBar-owned app and update surfaces; inherited AI-provider labels remain only where they describe provider functionality or upstream history.**
6. `Sources/CodexBar/StatusItemController.swift`: accessibility title and autosave names. **Done for ResearchBar.**
7. `docs/RELEASING.md`: ResearchBar prereqs (certs, ASC keys, feed). **Updated.**
8. `README.md`: user-facing install instructions. **Updated.**
9. Optional `Package.swift`: only if the SwiftPM product/module names are renamed (high churn; keep SwiftPM target names `CodexBar`, `CodexBarCore`, and `CodexBarCLI` stable through the first beta unless a later branch takes the full module rename).

Keep the SwiftPM target/module names (`CodexBar`, `CodexBarCore`) stable through the first beta if possible; shipped identity (display name, bundle id, feed, app executable, widget extension, helper names, command name) is what must change. This keeps upstream syncs cheap while letting ResearchBar run beside a real CodexBar install.
