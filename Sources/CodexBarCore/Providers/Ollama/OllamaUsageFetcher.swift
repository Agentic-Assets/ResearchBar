...
public enum OllamaUsageError: LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        switch self {
        case .missingAPIKey:
            "Missing Ollama API key. Set apiKey in ~/.config/researchbar/config.json or OLLAMA_API_KEY."
        case .notLoggedIn:
...
public enum OllamaAPIUsageFetcher {
        ...
        now: Date = Date()) async throws -> OllamaAPIUsageSnapshot
    {
        ...
        request.setValue("\(AppIdentity.displayName)/1.0", forHTTPHeaderField: "User-Agent")
