...
check_codex_parser_hash() {
  "${BASH_BIN}" "${ROOT_DIR}/Scripts/regenerate-codex-parser-hash.sh" --check
}

check_provider_manifests() {
  "${BASH_BIN}" "${ROOT_DIR}/Scripts/regenerate-provider-manifests.sh" --check
}

check_package_product_paths() {
  "${BASH_BIN}" "${ROOT_DIR}/Scripts/test_package_product_paths.sh"
}

check_package_strip() {
  "${BASH_BIN}" "${ROOT_DIR}/Scripts/test_package_strip.sh"
}

check_package_signing() {
  "${BASH_BIN}" "${ROOT_DIR}/Scripts/test_package_signing.sh"
}

check_package_info_plist() {
  "${BASH_BIN}" "${ROOT_DIR}/Scripts/test_package_info_plist.sh"
}

check_release_dsym_paths() {
  "${BASH_BIN}" "${ROOT_DIR}/Scripts/test_release_dsym_paths.sh"
}

check_sparkle_signing_paths() {
  "${BASH_BIN}" "${ROOT_DIR}/Scripts/test_sparkle_signing_paths.sh"
}

check_swift_test_sharding() {
  "${BASH_BIN}" "${ROOT_DIR}/Scripts/test_swift_test_sharding.sh"
}

check_ci_path_gate() {
  "${BASH_BIN}" "${ROOT_DIR}/Scripts/test_ci_path_gate.sh"
}

check_repository_size() {
  "${BASH_BIN}" "${ROOT_DIR}/Scripts/check_repository_size.sh"
  "${BASH_BIN}" "${ROOT_DIR}/Scripts/test_repository_size.sh"
}
